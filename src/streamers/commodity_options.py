...
from src.utils.commodity_contracts import load_commodity_options
...
def build_row(spec, ticker, mode):
    ...
    return (
        ...
        mode.upper(),
        'commodity',
        spec.metadata.get('instrument_type', 'option'),
        spec.metadata['display_name'],
        ticker.contract.symbol,
        None,
        ticker.contract.exchange,
        getattr(ticker.contract, 'lastTradeDateOrContractMonth', spec.metadata.get('expiry')),
        float(strike) if strike is not None else None,
        getattr(ticker.contract, 'right', spec.metadata.get('right')),
        spec.metadata.get('underlying', ticker.contract.symbol),
...
CONFIG = StreamerConfig(
    ...
    flow='options_all',
    description='Commodity options all streamer',
    load_specs=lambda args: load_commodity_options(args.expiry_month),
    build_row=build_row,
    needs_expiry=True,
)
